create_and_run_batch_file_upload_new <- function(AnaplanUser_x,WorkspaceID_x,ModelId_x,module_y){
wd1 <- getwd()
file_y <- paste(module_y,".csv", sep = "")
put_y <- paste(wd1,"/3_csv/",file_y,sep = "")
import_y <- paste(module_y," from ",file_y,sep = "")
output_y <- paste(wd1,"/5_errors/Anaplan_errors/",module_y,"_errors.csv",sep = "")
PathToBatchFile_y <- paste(wd1,"/4_anaplan-connect-1-3-3-3/",module_y,".bat",sep = "")
